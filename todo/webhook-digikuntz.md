...
- [x] **FIX timestamps anti-doublon** : `db.update_status_by_provider_id` (chemin
      webhook) pose désormais `cancelled_at=now()` (status cancelled) et
      `validated_at=now()` (status successful), comme `update_transaction`.
      (`ussd_sent_at` reste non posé — inconnu du webhook, vient du /charge.)
